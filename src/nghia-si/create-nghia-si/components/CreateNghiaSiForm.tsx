...
import { useCreateNghiaSi } from '../hooks/useCreateNghiaSi';
import {
...
function CreateNghiaSiForm() {
  ...
  const { mutate } = useCreateNghiaSi({
    onSuccess: () => {
      showSuccessSnackbar('Thêm mới đoàn sinh thành công');
      navigate(PATH_DASHBOARD.nghiaSi.list);
    },
    onError: () => {
  ...
  const { listClass } = useGetListClass({branchName: "NGHIA_SI"});
      ...
      <Stack direction="column" spacing={3}>
        ...
        <Stack justifyContent="flex-end" spacing={2} direction="row" sx={{ marinTop: 3 }}>
          ...
          <Button
            ...
            color="inherit"
            variant="contained"
            onClick={() => navigate(PATH_DASHBOARD.nghiaSi.list)}
          >
            {t('cancel')}
          </Button>
        </Stack>
...
export default CreateNghiaSiForm;
