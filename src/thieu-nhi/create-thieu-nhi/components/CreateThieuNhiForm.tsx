...
import { useCreateThieuNhi } from '../hooks/useCreateThieuNhi';
import {
...
function CreateThieuNhiForm() {
  ...
  const { mutate } = useCreateThieuNhi({
    onSuccess: () => {
      showSuccessSnackbar('Thêm mới đoàn sinh thành công');
      navigate(PATH_DASHBOARD.thieuNhi.list);
    },
    onError: () => {
  ...
  const { listClass } = useGetListClass({branchName: "THIEU_NHI"});
      ...
      <Stack direction="column" spacing={3}>
        ...
        <Stack justifyContent="flex-end" spacing={2} direction="row" sx={{ marinTop: 3 }}>
          ...
          <Button
            ...
            color="inherit"
            variant="contained"
            onClick={() => navigate(PATH_DASHBOARD.thieuNhi.list)}
          >
            {t('cancel')}
          </Button>
        </Stack>
...
export default CreateThieuNhiForm;
