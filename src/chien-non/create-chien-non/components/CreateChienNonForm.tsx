...
import { useCreateChienNon } from '../hooks/useCreateChienNon';
import {
...
function CreateChienNonForm() {
  ...
  const { mutate } = useCreateChienNon({
    onSuccess: () => {
      showSuccessSnackbar('Thêm mới đoàn sinh thành công');
      navigate(PATH_DASHBOARD.chienNon.list);
    },
    onError: () => {
...
export default CreateChienNonForm;
