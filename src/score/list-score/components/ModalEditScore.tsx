...
import { IDataEditScore, ISubmitDataEditScore } from 'src/score/common/interface';
import { useEditScore } from 'src/score/common/hooks/useEditScore';
...
export default function ModalEditScore({ id, name, lastName, classId, studentId }: Props) {
  ...
  const { mutate: mutateEdit } = useEditScore({
    onSuccess: () => {
  ...
  const methods = useForm<IDataEditScore>({
    defaultValues: {
      classId: classId,
      finalScore: 0,
      midScore: 0,
  ...
  const onSubmit = async (data: ISubmitDataEditScore) => {
    const editData: IDataEditScore = {
      classId: classId,
    ...
    mutateEdit({ formData: editData, id: id });
  };
