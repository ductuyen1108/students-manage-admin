...
import { useCreateAuNhi } from '../hooks/useCreateAuNhi';
import {
...
function CreateAuNhiForm() {
  ...
  const { mutate } = useCreateAuNhi({
    onSuccess: () => {
      showSuccessSnackbar('Thêm mới đoàn sinh thành công');
      navigate(PATH_DASHBOARD.auNhi.list);
    },
    onError: () => {
      ...
      <Stack direction="column" spacing={3}>
        ...
        <Stack justifyContent="flex-end" spacing={2} direction="row" sx={{ marinTop: 3 }}>
          ...
          <Button
            ...
            color="inherit"
            variant="contained"
            onClick={() => navigate(PATH_DASHBOARD.auNhi.list)}
          >
            {t('cancel')}
          </Button>
        </Stack>
...
export default CreateAuNhiForm;
